...
async function searchCabangByName(namacabang) {
  ...
  try {
    // Performing the findOne operation with the constructed query
    const cabang = await cabangCollection.findOne({ nama_cabang: namacabang })
    return cabang // Returning the found document or null if not found
  } catch (error) {
...
async function insertGudang(cabangId, gudangData) {
  ...
  const cabangCollection = mongodb.db('Toko').collection('Cabang')
  try {
    const cabangDocument = await cabangCollection.findOne({
      _id: Realm.BSON.ObjectId(cabangId)
    })
    if (!cabangDocument) {
      console.error('No document found with the specified cabangId:', cabangId)
      return
    } else {
      console.log('data nya ada')
    }
    const result = await cabangCollection.findOneAndUpdate(
      { _id: Realm.BSON.ObjectId(cabangId) },
    ...
    if (!result) {
      ...
      console.log('Gudang inserted successfully into Cabang.')
    }
  } catch (error) {
    console.error('Error inserting Gudang into Cabang:', error)
    throw error
  }
}
...
async function insertUser(cabangId, userData) {
  const mongodb = apl.currentUser.mongoClient('mongodb-atlas')
  const cabangCollection = mongodb.db('Toko').collection('Cabang')
  await cabangCollection.findOneAndUpdate(
    { _id: Realm.BSON.ObjectId(cabangId) },
    { $push: { Users: userData } }
  )
}
