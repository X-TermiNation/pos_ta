...
async function ShowItemFromCabang(id_cabang, id_gudang) {
  ...
  try {
    ...
    // Find the gudang in the cabang by ID
    const gudang = cabang.Gudang.find((g) => g._id === id_gudang)
    if (!gudang) {
...
async function SearchKategoriByName() {}
...
async function getfirstJenis() {
  let realm = await openRealm()
  try {
    const jenisDataAll = realm.objects('kategori')
...
async function SearchJenisByName() {}

async function addJenis() {}

async function searchjenisBykategori() {}
