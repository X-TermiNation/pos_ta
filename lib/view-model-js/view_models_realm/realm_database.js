...
const {
  DiskonSchema,
  DiskonItemsSchema
} = require('../models_realm/Diskon_realm')

const {
  TransaksiSchema,
  ItemsTransSchema,
  DeliverySchema
} = require('../models_realm/transaksi_realm')
...
// Update Barang
async function UpdateBarang(id_cabang, id_gudang, BarangData) {
  try {
    const cabang = realms.objectForPrimaryKey('Cabang', id_cabang)
    if (!cabang) {
      throw new Error('Cabang not found')
    ...
    const barang = gudang.Barang.find((b) =>
      b._id.equals(Realm.BSON.ObjectId(BarangData._id))
    )
    ...
    realms.write(() => {
      // Update only the fields that are provided in BarangData
      if (BarangData.nama_barang !== undefined) {
        barang.nama_barang = BarangData.nama_barang
      }
      if (BarangData.jenis_barang !== undefined) {
        barang.jenis_barang = BarangData.jenis_barang
      }
      if (BarangData.kategori_barang !== undefined) {
        barang.kategori_barang = BarangData.kategori_barang
      }
      if (BarangData.insert_date !== undefined) {
        barang.insert_date = BarangData.insert_date
      }
      if (BarangData.exp_date !== undefined) {
        barang.exp_date = BarangData.exp_date
          ? new Date(BarangData.exp_date)
          : undefined
      }
    })
...
async function addsatuan(id_cabang, id_gudang, id_barang, satuanData) {
  try {
    ...
    const convertedharga = parseFloat(satuanData.harga_satuan)
...
//delete satuan
async function deletesatuan(id_cabang, id_gudang, id_barang, id_satuan) {
  try {
    const cabang = realms.objectForPrimaryKey('Cabang', id_cabang)
    if (!cabang) {
      throw new Error('Cabang not found')
    ...
    const satuanIndex = barang.Satuan.findIndex((s) => s._id.equals(id_satuan))
    if (satuanIndex === -1) {
      throw new Error('Satuan not found in Barang')
    }

    realms.write(() => {
      barang.Satuan.splice(satuanIndex, 1) // Remove the satuan from the array
    })
    console.log('Satuan deleted successfully')
    return { id_satuan }
  } catch (error) {
    throw console.log('Failed to delete satuan:' + error)
  } finally {
    //realm.close()
  }
}
...
async function SatuanStockAlert(id_cabang, id_gudang) {
  try {
    const cabang = realms.objectForPrimaryKey('Cabang', id_cabang)
    if (!cabang) {
      throw new Error('Cabang not found')
    ...
    const lowStockBarang = []

    gudang.Barang.forEach((barang) => {
      barang.Satuan.forEach((satuan) => {
        if (satuan.jumlah_satuan < 1) {
          lowStockBarang.push({
            id_barang: barang._id,
            nama_barang: barang.nama_barang,
            id_satuan: satuan._id,
            nama_satuan: satuan.nama_satuan,
            jumlah_satuan: satuan.jumlah_satuan
          })
        }
      })
    })
    console.log(lowStockBarang)
    if (lowStockBarang.length > 0) {
      return lowStockBarang
    } else {
      console.log('No satuan with low stock found')
      return []
    }
  } catch (error) {
    console.error('Failed to fetch satuan data:', error)
    throw error
  }
}
