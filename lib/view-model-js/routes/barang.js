...
//belum
router.put('/updatebarang/:id_gudang/:id_cabang', async (req, res) => {
  const id_cabang = Realm.BSON.ObjectId(req.params.id_cabang)
  const id_gudang = Realm.BSON.ObjectId(req.params.id_gudang)
  const BarangData = req.body
  try {
    let updateBarang = await UpdateBarang(id_cabang, id_gudang, BarangData) // Delete the data_barang_ key from Redis
    await redis.del('data_barang_' + id_gudang)
    console.log(`Deleted key: data_barang_${id_gudang}`)
...
//delete satuan
router.delete(
  '/deletesatuan/:id_barang/:id_cabang/:id_gudang/:id_satuan',
  async (req, res) => {
    const id_barang = Realm.BSON.ObjectId(req.params.id_barang)
    const id_cabang = Realm.BSON.ObjectId(req.params.id_cabang)
    const id_gudang = Realm.BSON.ObjectId(req.params.id_gudang)
    ...
    try {
      const deletedSatuan = await deletesatuan(
        id_cabang,
        id_gudang,
        id_barang,
        id_satuan
      ...
      if (!deletedSatuan) {
        console.log('Satuan gagal dihapus')
        return res
          .status(404)
          .json({ status: 404, message: 'Satuan not found' })
      } else {
        console.log('Satuan berhasil dihapus')
      }
      ...
      res.status(200).json({
        status: 200,
        data: satuan2,
        message: 'Satuan deleted and data retrieved from the database'
      })
      redis.set(redisKey, JSON.stringify(satuan2))
    } catch (err) {
      console.log('Error deleting satuan: ' + err)
      res.status(500).json({ status: 500, message: 'Internal Server Error' })
    }
  }
)

//get low stock satuan
router.get('/getlowstocksatuan/:id_cabang/:id_gudang', async (req, res) => {
  try {
    const id_cabang = Realm.BSON.ObjectId(req.params.id_cabang)
    const id_gudang = Realm.BSON.ObjectId(req.params.id_gudang)

    const satuan = await SatuanStockAlert(id_cabang, id_gudang)
    if (satuan === 0) {
      console.log('data satuan tidak ditemukan')
    }
    // If data exists in Redis, send the cached data
