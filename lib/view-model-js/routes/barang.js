...
//barang
// router.get("/baranglist", async (req, res) => {
//   if (res.req.accepts('application/json')) {
//     res.setHeader('Content-Type', 'application/json');
//   }
//   try {
//     // Try to retrieve data from Redis
//     const cachedData = await redis.get('data_barang');
//     if (!cachedData) {
...
router.get("/baranglist/:id_gudang/:id_cabang", async (req, res) => {
  const id_gudang = req.params.id_gudang;
  const id_cabang = req.params.id_cabang;
  if (res.req.accepts('application/json')) {
    res.setHeader('Content-Type', 'application/json');
  }
  try {
    const cabang = await Cabang.find({_id:id_cabang});
    const gudang = cabang[0].Gudang;
    const barang = gudang[0].Barang;
    // Try to retrieve data from Redis
...
router.post("/addbarang/:id_gudang/:id_cabang", async (req, res) => {
  ...
  try {
    ...
    if (!gudang) {
      return res.status(404).json({ message: "Gudang not found within the Cabang" });
    }
    ...
    // Get all Barang in the Gudang and send the response
    const barang2 = gudang.Barang;
    res.status(200).json({
      status: 200,
      data: barang2,
      message: "Data retrieved from the database",
...
router.delete("/deletebarang/:id_gudang/:id_cabang/:id_barang", async (req, res) => {
  const { id_gudang, id_cabang, id_barang } = req.params;
  try {
    const cabang = await Cabang.findById(id_cabang);
    if (!cabang) {
      return res.status(404).json({ message: "Cabang not found" });
    ...
    const barangIndex = gudang.Barang.findIndex(barang => barang._id == id_barang);
    if (barangIndex === -1) {
      return res.status(404).json({ message: "Barang not found within the Gudang" });
    }
...
//
router.put("/updatebarang/:id_gudang/:id_cabang/:barangId", async (req, res) => {
  ...
  try {
    ...
    const gudang = cabang.Gudang.id(id_gudang);
    if (!gudang) {
      return res.status(404).json({ message: "Gudang tidak ditemukan dalam Cabang" });
    }

    const barang = gudang.Barang.id(barangId);
    if (!barang) {
      return res.status(404).json({ message: "Barang tidak ditemukan dalam Gudang" });
    }
...
//satuan
//blm dicoba
router.get("/getsatuan/:id_barang/:id_cabang", async (req, res) => {
  try {
    const { id_barang, id_cabang } = req.params;

    const redisKey = `data_satuan_${id_barang}_${id_cabang}`;
    ...
    if (!cachedData) {
      ...
      if (!gudang) {
        return res.status(404).json({ message: "Gudang not Found in the Cabang" });
      }

      // Find the specific barang in the gudang
      const barang = gudang.Barang.find(item => item._id.toString() === id_barang);
      if (!barang) {
        return res.status(404).json({ message: "Barang not Found in the Gudang" });
      }
      ...
      // Store the data in Redis for future use with the unique key
...
router.post("/addsatuan/:id_barang/:id_cabang", async (req, res) => {
  const {id_barang,id_cabang} = req.params;
  try {
    const cabang = await Cabang.findById(id_cabang);
    ...
    if (!gudang) {
      return res.status(404).json({ message: "Gudang not found with the specified Barang" });
    }
    ...
    // Save the updated cabang document
    await cabang.save();
    res.status(200).json({
      status: 200,
      data: satuan,
    ...
    redis.del(redisKey, function(err, reply) {
      if (err) {
          ...
          console.log(`Deleted key: data_barang`);
      }
...
router.post("/tambahjenis", async (req, res) => {
  try {
    ...
    let nmjenis = req.body.nama_jenisi;
...
//diskon
router.post("/tambahdiskon", async (req, res) => {
  if (res.req.accepts('application/json')) {
    res.setHeader('Content-Type', 'application/json');
  }
  try {
    let diskon = new Diskon(req.body);
    const existingData = await Diskon.findOne({ nama_diskon : diskon.nama_diskon });
    if (!existingData) {
      diskon = await diskon.save();
      res.status(200).json({
        status: 200,
        data: diskon,
      });
...
//
router.get("/diskonlist", async (req, res) => {
  if (res.req.accepts('application/json')) {
    res.setHeader('Content-Type', 'application/json');
  }
  try {
    // Try to retrieve data from Redis
    const cachedData = await redis.get('data_diskon');
    if (!cachedData) {
      // If data is not in Redis, query the database
      let diskon = await Diskon.find();
      ...
      // Store the data in Redis for future use
      redis.set('data_diskon', JSON.stringify(diskon));
      console.log("ini kosong redis");
    } else {
      // If data exists in Redis, send the cached data
      res.status(200).json({
...
router.get("/diskonlist/:nama_diskon", async (req, res) => {
  ...
  try {
    const { nama_diskon } = req.params;
      let diskon = await Diskon.findOne({nama_diskon:nama_diskon});
      // Send the data to the client
      res.status(200).json({
        status: 200,
        data: diskon,
...
router.get("/listdiskonbarang/:id_diskon", async (req, res) => {
  ...
  try {
    const { id_diskon } = req.params;
    const cachedData = await redis.get('data_diskon_item');
    if (!cachedData) {
      let diskon = await Diskon_item.find({id_diskon:id_diskon});
      // Send the data to the client
      res.status(200).json({
        status: 200,
        data: diskon,
...
//selesaikan ini dlu
router.post("/tambahdiskonbarang/:id_diskon/:id_barang", async (req, res) => {
  const {id_diskon,id_barang} = req.params;
  try {
    const diskon = await Diskon.findById(id_diskon);
    if (!diskon) {
      return res.status(404).json({ message: "Diskon tidak ditemukan" });
    }
    const barang = req.body;
    const existingData = diskon.Barang.find(barang => String(barang._id) === id_barang);
    if (!existingData) {
      if (res.req.accepts('application/json')) {
        res.setHeader('Content-Type', 'application/json');
      }
      ...
      let diskonbarang2 = await Diskon.find();
        ...
        // Store the data in Redis for future use
        redis.set('data_diskon', JSON.stringify(diskonbarang2));
