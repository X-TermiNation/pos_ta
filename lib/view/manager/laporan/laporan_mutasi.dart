...
class MutasiReportPage extends StatefulWidget {
  const MutasiReportPage({super.key});
...
class _MutasiReportPageState extends State<MutasiReportPage> {
  ...
  Future<void> _printPdf() async {
    ...
    try {
      final now = DateTime.now();
      final namaFile =
          'Laporan_Mutasi_${DateFormat('dd-MM-yyyy').format(now)}_${DateFormat('HHmmss').format(now)}.pdf';
  ...
  Future<pw.Document> _generatePdf(Map<String, dynamic> data) async {
    ...
    pw.Widget buildMutasiTable(List<dynamic> mutasiList) {
      if (mutasiList.isEmpty) return pw.Text('Tidak terdapat mutasi.');
    ...
    pdf.addPage(
      pw.MultiPage(
        ...
        build: (context) => [
          ...
          pw.Text('Mutasi Keluar',
              style:
                  pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold)),
          pw.SizedBox(height: 4),
          buildMutasiTable(mutasiKeluar),
          pw.Text('Mutasi Masuk',
              style:
                  pw.TextStyle(fontSize: 16, fontWeight: pw.FontWeight.bold)),
          pw.SizedBox(height: 4),
  ...
  Widget _buildMutasiList(String title, List<dynamic> list) {
    if (list.isEmpty) return Text('Tidak ada data $title.');
                ...
                child: Row(
                  ...
                  children: [
                    Expanded(
                      child: SingleChildScrollView(
                        child: _buildMutasiList(
                            'Mutasi Masuk', mutasiData!['mutasi']['masuk']),
                      ),
                    ),
                    ...
                    Expanded(
                      child: SingleChildScrollView(
                        child: _buildMutasiList(
                            'Mutasi Keluar', mutasiData!['mutasi']['keluar']),
                      ),
                    ),
                  ],
                ),
